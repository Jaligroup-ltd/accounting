...
def _service_url():
	return (frappe.conf.get("imbank_service_url") or "http://127.0.0.1:8899").rstrip("/")
...
def _do_fetch(from_date, to_date, user):
	...
	try:
		...
		if resp.status_code == 409:
			_toast(user, "A fetch is already running. Try again shortly.", indicator="orange")
			return
		...
		_toast(user, "I&M fetch started , you'll be notified when it's done.", indicator="blue")
...
def notify_fetch_done(success=False, detail=""):
	...
	msg = (
		"I&M import complete , click 'Get Unreconciled Entries'."
		if success
		else f"I&M import failed: {detail}"
	)
